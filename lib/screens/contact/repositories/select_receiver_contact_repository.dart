...
class SelectReceiverContactsRepository {
  ...
  Future<List<app.User>> getReceiverContacts(BuildContext context, Ref ref) async {
    List<app.User> contactsList = [];
    try {
      var usersData =
      ...
      for (var userMap in usersData.docs) {
        try {
          ...
          contactsList.add(app.User.fromMap(userMap.data()));
        } catch (err) {
  ...
  //checking if the receiver and sender are same or not
  bool isContactSame(currentUsername, receiverUsername){
    return currentUsername == receiverUsername;
  }
  ...
  //     if (username == receiverUser.username) {
  //       isFound = true;
  ...
  //           StringsConsts.username: receiverUser.username,
